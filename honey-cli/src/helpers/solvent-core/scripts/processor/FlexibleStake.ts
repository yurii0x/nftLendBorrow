...
import { flexibleStakeInstruction } from "../rpc/ixbuilder/flexibleStake";
import { Wallet } from "../types/Wallet";

export type FlexibleStakeValidationResponse = {
  status: boolean;
  reason: String;
};

export const flexibleStakeWrapper = async (
  provider: Provider,
  amount: number
): Promise<RPCResponse> => {
  try {
    ...
    const dataVars: FlexibleStakeVars = await fetchDataVars(
      provider,
    ...
    const ix: TransactionInstruction = await flexibleStakeInstruction(
      provider,
      dataVars
    );
...
export const performPreRPCValidations = async (
  provider: Provider,
  userWallet: Wallet
): Promise<FlexibleStakeValidationResponse> =>
