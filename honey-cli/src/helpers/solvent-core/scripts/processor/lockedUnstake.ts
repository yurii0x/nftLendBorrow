...
import { fetchDataVars } from "../data/fetcher/lockedUnstake";
...
import { lockedUnstakeInstruction } from "../rpc/ixbuilder/lockedUnstake";
import { Wallet } from "../types/Wallet";

export type LockedUnstakeValidationResponse = {
  status: boolean;
  reason: String;
};

export const lockedUnstakeWrapper = async (
  provider: Provider,
  amount: number
): Promise<RPCResponse> => {
  try {
    ...
    const dataVars: LockedUnstakeVars = await fetchDataVars(
      provider,
    ...
    const ix: TransactionInstruction = await lockedUnstakeInstruction(
      provider,
      dataVars
    );
...
export const performPreRPCValidations = async (
  provider: Provider,
  userWallet: Wallet
): Promise<LockedUnstakeValidationResponse> =>
