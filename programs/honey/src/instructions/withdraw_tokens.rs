...
use anchor_spl::token::{self, Burn, Token, Mint, TokenAccount, Transfer};
...
pub struct WithdrawTokens<'info> {
    ...
    #[account(mut,
        ...
        bump)]
    pub deposit_note_account: Account<'info, TokenAccount>,
    ...
    fn note_burn_context(&self) -> CpiContext<'_, '_, '_, 'info, Burn<'info>> {
        CpiContext::new(
            ...
            Burn {
                ...
                authority: self.market_authority.clone(),
                from: self.deposit_note_account.to_account_info(),
            },
        )
    }
}
...
pub fn handler(ctx: Context<WithdrawTokens>, _bump: u8, amount: Amount) -> Result<()> {
    ...
    token::burn(ctx.accounts.note_burn_context()
        .with_signer(&[&market.authority_seeds()]), note_amount)?;
