...
pub struct Market {
    ...
    pub nft_switchboard_price_aggregator: Pubkey,

    /// The mint for the token being held in this reserve
    pub nft_collection_creator: Pubkey,

    /// Storage for flags that can be set on the market.
    pub flags: u64,

    /// oracle price data
    pub market_oracle_state: [u8; 24],
...
impl Market {
    ...
    pub fn market_oracle_mut(&mut self) -> &mut MarketOracle {
        bytemuck::from_bytes_mut(&mut self.market_oracle_state)
    }

    pub fn market_oracle(&self) -> &MarketOracle {
        bytemuck::from_bytes(&self.market_oracle_state)
    }
    ...
    pub fn verify_ability_deposit_withdraw(&self) -> Result<()> {
    ...
    pub fn verify_ability_borrow(&self) -> Result<()> {
    ...
    pub fn verify_ability_repay(&self) -> Result<()> {
...
impl MarketReserves {
    pub fn register(&mut self, reserve: &Pubkey) -> Result<ReserveIndex> {
...
impl std::fmt::Debug for Market {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("Market")
            ...
            .field("nft_collection_creator", &self.nft_collection_creator)
            .finish()
    }
}
