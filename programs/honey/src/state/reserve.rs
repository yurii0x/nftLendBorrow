...
pub struct Reserve {
    ...
    pub protocol_fee_note_vault: Pubkey,
...
impl Reserve {
    ...
    /// Calculates the protocol borrow fee
    pub fn protocol_fee(&self, token_amount: u64) -> u64 {
        let origination_fee = Number::from_bps(150);
        let fee_owed = origination_fee * token_amount;
    ...
    pub fn borrow(
        ...
        let protocol_fees = Number::from_decimal(protocol_fees, 0);

        state.uncollected_fees += fees;
        state.protocol_uncollected_fees += protocol_fees;
        state.outstanding_debt += borrowed_amount + fees + protocol_fees;
        state.total_deposits = state.total_deposits.checked_sub(token_amount).unwrap();
    ...
    /// might have to add protocol fees also TBD during liquidation
    ...
    pub fn deposit_note_exchange_rate(
        ...
        let calc = DepositNoteCalculator {
            ...
            protocol_uncollected_fees: state.protocol_uncollected_fees,
            vault_total,
            mint_supply,
        };
    ...
    pub fn try_accrue_interest(
        ...
        match time_to_accrue.cmp(&0) {
            ...
            Ordering::Greater => {
                ...
                //yes interest on protocol interest also needs to be added - make this change
    ...
    /// Collect any protocol fees that were accumulated
    ///
    /// Returns the number of notes to mint to represent the fees collected
    pub fn collect_accrued_protocol_fees(
        &mut self,
        current_slot: u64,
        ...
        if threshold > state.protocol_uncollected_fees {
            // not enough accumulated to be worth minting new notes for
            return 0;
        }

        let fee_notes = (state.protocol_uncollected_fees / exchange_rate).as_u64(0);

        state.protocol_uncollected_fees = Number::ZERO;
        state.total_deposit_notes = state.total_deposit_notes.checked_add(fee_notes).unwrap();
...
#[assert_size(aligns, 520)]
#[derive(Pod, Zeroable, Clone, Copy)]
#[repr(C)]
struct ReserveState {
    accrued_until: i64, //64

    outstanding_debt: Number,

    uncollected_fees: Number,

    protocol_uncollected_fees: Number,
...
impl<'a> NoteCalculator for DepositNoteCalculator {
    ...
    fn token_supply(&self) -> Number {
        ...
        self.outstanding_debt + Number::from(self.vault_total)
            - self.uncollected_fees
            - self.protocol_uncollected_fees
    }
}
...
pub trait NoteCalculator {
    ...
    fn notes_from_tokens(&self, tokens: u64) -> u64 {
        ...
        msg!("notes_from_tokens: tokens={:?}", tokens);
        let notes = tokens / self.exchange_rate();
        msg!("notes_from_tokens: notes={:?}", notes);
...
mod tests {
    ...
    fn sane_deposit_note_exchange_rate() {
        ...
        let protocol_uncollected_fees = Number::from(0);
