...
// export const DEX_PID = new PublicKey("BzcdcuWRSGhsLNrKvPTRubVLS19UmgxvxuKYYxuiPB7Q"); // devnet

export async function setupTwoMarkets({ provider, bucketMint=null, godA=null }) {
  // Setup mints with initial tokens owned by the provider.
  console.log("wallet when setting 2 markets", provider.wallet.publicKey.toString())
  const decimals = 8;
  let [MINT_A, GOD_A] = await createMintAndVault(
    provider,
  ...
  if(bucketMint != null && godA != null) {
    MINT_A = bucketMint;
    GOD_A = godA;
  }

  const [MINT_B, GOD_B] = await createMintAndVault(
    provider,
    new BN(1000000000000000),
    provider.wallet.publicKey,
    decimals
  );
  const [USDC, GOD_USDC] = await createMintAndVault(
    provider,
    new BN(1000000000000000),
    provider.wallet.publicKey,
    decimals
  ...
  const MARKET_B_USDC = await setupMarket({
    baseMint: MINT_B,
    quoteMint: USDC,
    marketMaker: {
      account: marketMaker.account,
...
async function initOrderbook({ provider, bids, asks }) {
  ...
  const decimals = 8;
  const [MINT_A, GOD_A] = await createMintAndVault(
    provider,
    new BN(1000000000000000),
    undefined,
    decimals
...
async function listMarket({
  ...
  console.log('programID of DexInstructions', tx2.instructions[5].programId.toString());
